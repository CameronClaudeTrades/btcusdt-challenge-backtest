...
TELEGRAM_TOKEN = "8602400194:AAFIurwmOgBWV8XSDfpQre5C-stwwq3M60I"
TELEGRAM_CHAT_ID = "8566409389"
...
def send_telegram(text):
    try:
        ...
        print("TELEGRAM SEND:", r.status_code, r.text[:300])
        return r.status_code, r.text[:300]
    except Exception as e:
        print("TELEGRAM SEND EXCEPTION:", type(e).__name__, e)
        return "EXC", str(e)
...
def run_backtest():
    lines = []
    lines.append("Fetching ~4 months of historical data...")
    c5 = fetch_all_klines("5m", LOOKBACK_DAYS)
    c15 = fetch_all_klines("15m", LOOKBACK_DAYS)
    lines.append(f"5m candles: {len(c5)}, 15m candles: {len(c15)}")
    print(lines[-1])
    ...
    lines.append(f"Total signals found: {len(trades)}  Wins: {wins}  Losses: {losses}")
    ...
    lines.append("=== CHALLENGE SIMULATION ($5,000 account, 12% target, 6% max DD, 3% daily DD) ===")
    lines.append(f"Data window: last {LOOKBACK_DAYS} days of real BTCUSDT 5m/15m data")
    lines.append("Position size 40% of balance, SL 0.4%, RR 1:4 (risk ~0.16% of equity per trade, compounding)")
    lines.append(f"Elapsed simulated days before stopping: {elapsed_days:.1f}")

    if target_hit is not None:
        lines.append("OUTCOME: PASSED -- profit target reached before any drawdown breach")
        lines.append(f"Final equity: {target_hit['equity']:.2f} (+{(target_hit['equity']/START_BALANCE-1)*100:.2f}%)")
    elif breach is not None:
        lines.append(f"OUTCOME: FAILED -- breached {breach['type']} drawdown limit before reaching target")
        lines.append(f"Equity at breach: {breach['equity']:.2f}  Overall DD: {breach['overall_dd']*100:.2f}%  Daily DD: {breach['daily_dd']*100:.2f}%")
    else:
        lines.append("OUTCOME: NEITHER -- ran out of data without hitting target or breaching limits")
        lines.append(f"Final balance: {balance:.2f} ({(balance/START_BALANCE-1)*100:+.2f}%)")

    lines.append(f"Max overall drawdown observed: {max_overall_dd*100:.2f}%")
    lines.append(f"Max single-day drawdown observed: {max_daily_dd*100:.2f}%")
    lines.append(f"Trades taken before stop: {trade_ptr}")
...
send_telegram("Backtest starting via GitHub Actions (4-month BTCUSDT challenge sim)... this will take a minute or two.")

try:
    summary = run_backtest()
except Exception:
    summary = "BACKTEST ERROR:\n" + traceback.format_exc()
    print(summary)

send_telegram("BTCUSDT Challenge Backtest (4 months)\n\n" + summary)
